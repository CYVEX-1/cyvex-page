			...
			<div className="mx-auto w-full max-w-7xl px-4 sm:px-6">
				<div className="mx-auto max-w-2xl text-center">
					<span className="inline-flex items-center gap-2 rounded-full border border-border/60 bg-background/60 px-3 py-1 font-medium text-cyvex-cyan-strong text-xs backdrop-blur">
						<span className="size-1.5 rounded-full bg-cyvex-cyan" /> Sobre a
						Cyvex
					</span>
					<h2 className="mt-5 text-balance font-semibold text-3xl tracking-tight sm:text-4xl lg:text-5xl">
						Autoridade em{" "}
						<span className="text-gradient-cyber">cibersegurança</span> para um
						mundo em constante ataque.
					</h2>
					<p className="mt-5 text-pretty text-base text-muted-foreground leading-relaxed sm:text-lg">
						Combinamos uma equipe sênior de ofensores e defensores com
						metodologias modernas para entregar proteção real — não apenas
						relatórios. Atuamos lado a lado com clientes corporativos e pessoas
						físicas em cada etapa da jornada de segurança.
					</p>
				</div>
				...
				<div className="mt-14 grid gap-5 md:grid-cols-3">
					{PILLARS.map(({ icon: Icon, title, description }) => (
						...
						>
							...
							<div className="pointer-events-none absolute -right-12 -bottom-12 size-40 rounded-full bg-cyvex-cyan/8 opacity-0 blur-2xl transition-opacity group-hover/p:opacity-100" />
						</article>
					))}
				</div>
				...
				<div className="mt-12 grid gap-px overflow-hidden rounded-2xl border border-border/60 bg-border/60 sm:mt-16 md:grid-cols-2 xl:grid-cols-4">
					{HIGHLIGHTS.map(({ icon: Icon, title, description }) => (
						...
						>
							...
							<h4 className="mt-4 font-semibold text-base tracking-tight">
								{title}
							</h4>
