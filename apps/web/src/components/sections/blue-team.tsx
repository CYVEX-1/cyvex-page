		...
		>
			...
			<div className="mx-auto w-full max-w-7xl px-4 sm:px-6">
				<div className="grid gap-10 lg:grid-cols-[1fr_1.4fr] lg:items-end">
					<div>
						<span className="inline-flex items-center gap-2 rounded-full border border-cyvex-cyan/25 bg-cyvex-cyan/8 px-3 py-1 font-medium text-cyvex-cyan-strong text-xs">
						...
						<h2 className="mt-4 text-balance font-semibold text-3xl tracking-tight sm:text-4xl lg:text-5xl">
							Defesa <span className="text-gradient-cyber">corporativa</span> de
							elite, em camadas.
						</h2>
					</div>
				...
				<div className="mt-14 grid gap-5 lg:grid-cols-3">
					...
					/>
					<SocMetric
						label="Incidentes contidos"
						value="1.328"
						trend="MTTR < 15 min"
						highlight
					/>
					<SocMetric
						label="Cobertura de ativos"
						value="100%"
						trend="visibilidade total"
					/>
				</div>
			</div>
		</section>
		...
		>
			<div className="flex items-center justify-between text-[10px] text-muted-foreground uppercase tracking-widest">
				...
				<span className="inline-flex items-center gap-1 text-cyvex-cyan">
					<span className="size-1.5 animate-pulse rounded-full bg-cyvex-cyan" />
					live
				</span>
			</div>
			<div className="mt-4 font-semibold text-3xl tracking-tight sm:text-4xl">
				<span className={highlight ? "text-gradient-cyber" : "text-foreground"}>
					{value}
				</span>
			</div>
			<div className="mt-1 text-muted-foreground text-sm">{trend}</div>
			<div className="pointer-events-none absolute -top-8 -right-8 size-32 rounded-full bg-cyvex-cyan/10 blur-2xl" />
		</div>
	);
}
