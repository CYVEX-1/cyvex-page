...
export function Investigation() {
  return (
    <section id="investigacao" className="relative isolate overflow-hidden py-24 sm:py-32">
      <CyberBg variant="aurora" />

      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6">
        <div className="mx-auto max-w-3xl text-center">
          <span className="inline-flex items-center gap-2 rounded-full border border-cyvex-blue/30 bg-cyvex-blue/10 px-3 py-1 text-xs font-medium text-cyvex-blue">
            <span className="size-1.5 rounded-full bg-cyvex-blue" /> Investigação Social
          </span>
          <h2 className="mt-5 text-balance text-3xl font-semibold tracking-tight sm:text-4xl lg:text-5xl">
            <span className="text-gradient-cyber">Inteligência digital</span> aplicada à proteção
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-2">
          {AUDIENCES.map(({ icon: Icon, badge, title, description, bullets }) => (
            <div
              key={title}
              className="glass relative overflow-hidden rounded-3xl border border-border/70 p-7 sm:p-9"
            >
              <div className="pointer-events-none absolute -right-16 -top-16 size-56 rounded-full bg-cyvex-cyan/12 blur-3xl" />
              <span className="inline-flex items-center gap-2 rounded-full border border-cyvex-cyan/25 bg-cyvex-cyan/8 px-3 py-1 text-[11px] font-medium text-cyvex-cyan-strong">
              ...
              <Link
                href="#contato"
                className="mt-7 inline-flex items-center gap-1.5 text-sm font-semibold text-cyvex-cyan-strong transition-colors hover:text-cyvex-cyan"
              >
